<!DOCTYPE HTML>
<html>
	<head>
			<?php require_once('contents/style.php'); ?>
	</head>

	<body>
			<?php require_once('navbar.php'); ?>


			   <div id="index-banner" class="parallax-container align-wrapper">
			    <div class="section no-pad">
			      <div class="container" id="banner1">
			        <h1 class="header center grey-text">Jugement Majoritaire</h1>
			        <div class="row center">
			          <h5 class="header col s12 black-text ">Le vote numérique qui change tout</h5>
			        </div>

			      </div>
			    </div>
			    <div class="parallax">
			    	<img src="../contents/images/background1.jpg" alt="Unsplashed background img 1">
			    </div>
			  </div>

			  <div class="container">
			  	
			  	<h2 class="green-text">  <i class="medium material-icons prefix">done</i>Vote enregistré ! </h2>
			  	<p>Nous vous remercions d'avoir choisi cette platerforme afin de réaliser ce vote. À très bientôt !  </p>
			</div>

			<div class="row center">
				 <a  href="/resultats/afficher" class="waves-effect waves-light btn light-blue lighten-2">Resultats</a>
			</div>


 		<?php require 'footer.php'; ?>

 			<?php require_once('contents/script.php'); ?>
				
	</body>


	

       

</html>